...
int main(int argc, char *argv[])
{
	...
	for (int y = 0; y < 16; y++) {
		for (int x = 0; x < 16; x++) {
			unsigned long int l = grid[y][x] * grid[y][x + 1] * grid[y][x + 2] * grid[y][x + 3];
			unsigned long int u = grid[y][x] * grid[y + 1][x] * grid[y + 2][x] * grid[y + 3][x];
			unsigned long int d1 = grid[y][x] * grid[y + 1][x + 1] * grid[y + 2][x + 2] * grid[y + 3][x + 3];
			unsigned long int d2 = grid[y][x + 3] * grid[y + 1][x + 2] * grid[y + 2][x + 1] * grid[y + 3][x];
			ans = MAX(ans, MAX(u, MAX(l, MAX(d1, d2))));
		}
	}
